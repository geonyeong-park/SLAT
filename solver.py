...
class Solver(object):
    def __init__(self, basemodel, C1, C2, netDFeat, loader, TargetLoader,
        ...
        self.log_loss = {}
        self.log_loss['Dloss'] = []
        self.log_loss['domain_size'] = []
        self.log_loss['domain_correct'] = []
    ...
    def train(self):
        ...
        stop_step = self.config['train']['num_steps_stop']
        sample_path = os.path.join(self.log_dir, '{}_log.pkl')
        ...
        for i_iter in range(self.total_step):
            ...
            p = float(i_iter) / stop_step
            constant = 2. / (1. + np.exp(-10 * p)) - 1

            self._train_step(i_iter, constant)
            ...
            if (i_iter+1) % self.val_step == 0:
                ...
                with open(sample_path.format(i_iter+1), 'wb') as f:
    ...
    def _train_step(self, i_iter, alpha):
        ...
        _, DFeatlogit = self.basemodel(images, alpha)
        ...
        self.log_loss['Dloss'].append(bloss_AdvFeat.item())
        self.optBase.step()
        ...
        if (i_iter+1) % self.log_step == 0:
            ...
            for tag, value in self.log_loss.items():
                log += ", {}: {:.4f}".format('Dloss', bloss_AdvFeat.item())
            h, _ = self.basemodel(images)
            pred = self.C1(h)
            ...
            # --------------------------------
            domain_correct = np.zeros(5)
            size = np.zeros(5)
            add_more = [True for _ in range(self.num_domain)]
            loaders = self.loader.loader_list

            loader_iter_ = iter(self.loader)
            for i in range(max(self.len_of_domain) // self.batch_size):
                images, labels = next(loader_iter_)
                images = Variable(images.to(torch.float).detach())
                labels = Variable(labels.long().detach())
                images = images.to(self.gpu0)
                labels = labels.to(self.gpu0)
                _, output = self.basemodel(images)

                for d in range(self.num_domain):
                    if self.len_of_domain[d] > size[d] + self.batch_size and add_more[d]:
                        size[d] += self.batch_size

                        pred = output.data[d*self.batch_size: (d+1)*self.batch_size].max(1)[1]
                        domain_label = torch.tensor([d for _ in range(self.batch_size)], dtype=torch.long).to(self.gpu0)
                        domain_correct[d] += pred.eq(domain_label.data).cpu().sum()
                    elif self.len_of_domain[d] > size[d] and add_more[d]:
                        remainder = int(self.len_of_domain[d] - size[d])
                        size[d] += remainder
                        add_more[d] = False

                        pred = output.data[d*self.batch_size: d*self.batch_size+remainder].max(1)[1]
                        domain_label = torch.tensor([d for _ in range(remainder)], dtype=torch.long).to(self.gpu0)
                        domain_correct[d] += pred.eq(domain_label.data).cpu().sum()
                    else:
                        continue
            domain_acc = domain_correct / size
            self.log_loss['domain_size'].append(size)
            self.log_loss['domain_correct'].append(domain_correct)
            print(size)
            print(domain_acc)
